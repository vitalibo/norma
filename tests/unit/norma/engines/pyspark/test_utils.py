...
@pytest.mark.parametrize(
    ('case', 'column'), [
        pytest.param(f'case{i}', column, id=f'Case #{i}: {name}')
...
def test_nested_set_expr(case, column, spark):
    ...
    spark.assert_dataframe_equals(actual, expected, schema=schema)


@pytest.mark.parametrize(
    ('column', 'root_expr', 'expected'), [
        pytest.param(column, params[0] if params else lambda: None, expected, id=f'Case #{i}: {name}')
        for i, (name, column, expected, *params) in enumerate([
            ('top-level', 'f1', ['v1', 'v2']),
            ...
            (
                'use root_expr instead of fn.col of the root',
                'f2.f3.f4',
                ['A', None],
                lambda: fn.col('f2').withField(
                    'f3', fn.col('f2').getField('f3').withField(
                        'f4', fn.upper(fn.col('f2').getField('f3').getField('f4'))
                    )
                ),
            ),
        ])
...
def test_nested_drop_expr(case, column, spark):
    ...
    root = column.split('.')[0].removesuffix('[]')
    actual = df.withColumn(root, nested_drop_expr(column, fn.col(root), df.schema[root].dataType))
